...
struct Extension_info { // información global acerca de una extensión determinada
(.exe, .txt, etc)
std::uintmax_t num_files, // número total de archivos con una extensión
determinada
total_size; // tamaño total de bytes acumulado por dicha
extensión
};
auto main() -> int
{
...
// mapa global que asocia a cada extensión el número total de archivos y el
tamaño
...
// fs::is_regular_file y fs::is_directory para determinar si la ruta
refiere
// un fichero o un directorio, respectivamente. Si se trata de un fichero,
emplearemos
// su extensión pth.extension().string() y tamaño en memoria
fs::file_size(pth)
// para actualizar el mapa res. Si pth es es un directorio,
incrementaremos el
...
// ellos; mientras se ejecutan en paralelo los hilos lanzados, main se encargará
de
...
auto root_file = std::uintmax_t{0}; // número total de archivos dentro de la
ruta raíz
